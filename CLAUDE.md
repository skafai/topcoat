...
Always use the `commit` skill before authoring any commit. Never include GSD phase numbers (e.g. `feat(02-02):`), GSD labels (e.g. `CR-01`, `WR-02`), or "phase XX" references in commit messages. Use meaningful scope names (e.g. `feat(mdx): add component embedding`). Respect `commit_docs: false` in config: never commit `.planning/` files.
