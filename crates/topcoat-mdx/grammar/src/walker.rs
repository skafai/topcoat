...
use topcoat_view_grammar::{
    ...
    view::{
        ...
        Nodes, OpeningTag, SelfClosingTag, View,
    },
};
...
///
/// # Panics
/// Panics if an override is registered for a heading tag
/// (verified via `ctx.overrides.iter().any(...)`) but `try_apply_override`
/// returns `None` — this should not happen when the guard is used, as both
/// check the same `ctx.overrides` slice.
